...
Future<void> validateDocumentImport(
  Uint8List bytes,
  String name,
  VaultDocumentFileType type,
...
void _validate((Uint8List, String, VaultDocumentFileType) input) {
  ...
  if (ext == '.heic' || ext == '.heif') {
    if (header.length < 12 ||
        ...
        ![
          ...
          'mif1',
          'msf1',
        ].contains(header.substring(8, 12))) {
  ...
  try {
    ...
    if (info == null ||
        info.width <= 0 ||
        info.height <= 0 ||
        info.width * info.height > DocumentImportLimits.imagePixels ||
        info.numFrames > 100) {
      throw const FormatException(
        'Use an image up to 40 megapixels and 100 frames.',
      );
    }
  } on FormatException {
    rethrow;
  } catch (_) {
    throw const FormatException('This image is damaged or unsupported.');
  }
}
