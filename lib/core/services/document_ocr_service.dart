...
class DocumentOcrService {
  ...
  Future<String> extract(VaultDocument document, {
    bool Function()? isCancelled,
    void Function(int, int)? onProgress,
  }) async {
    void check() {
      storage.requireUnlocked?.call();
      if (isCancelled?.call() == true) throw StateError('Text extraction cancelled.');
    }
    Future<String> recognize(Uint8List image) async {
      check();
      final text = await channel.invokeMethod<String>('recognize', {'bytes': image}) ?? '';
      check();
      return text;
    }
    check();
    final bytes = await storage.readDecryptedBytes(document.filePath);
    if (bytes.length > 20 * 1024 * 1024) throw const FormatException('Text extraction supports files up to 20 MB.');
    if (document.fileType == VaultDocumentFileType.image) {
      ...
      if (text.length > 1024 * 1024) throw const FormatException('Extracted text exceeds the supported size.');
      onProgress?.call(1, 1);
      return text;
    }
    if (document.fileType != VaultDocumentFileType.pdf) throw const FormatException('Choose an image or PDF for text extraction.');
    final pdf = await PdfDocument.openData(bytes);
    try {
      if (pdf.pagesCount > 20) throw const FormatException('Text extraction supports PDFs up to 20 pages. Split this PDF first.');
      ...
      for (var i = 1; i <= pdf.pagesCount; i++) {
        ...
        try {
          ...
          final rendered = await page.render(width: page.width * scale,
            height: page.height * scale, format: PdfPageImageFormat.png, backgroundColor: '#FFFFFF');
          if (rendered == null) throw const FormatException('A PDF page could not be rendered.');
          result.writeln(await recognize(rendered.bytes));
          if (result.length > 1024 * 1024) throw const FormatException('Extracted text exceeds the supported size.');
...
Uint8List _normalizeImage(Uint8List bytes) {
  ...
  if (first == null) throw const FormatException('The image could not be decoded.');
  var normalized = img.bakeOrientation(first);
  if (normalized.width > 2400 || normalized.height > 2400) {
    normalized = img.copyResize(normalized,
